...
function resetPw(){
  ...
  const np = $('fgNew').value;
  if(!np){ toast('請輸入新密碼'); return; }
  const cfg = DB.getCfg(); cfg.pw = hash(np); DB.setCfg(cfg);
...
function nav(view){
  closeDrawer();
  document.querySelectorAll('.drawer .di[data-view]').forEach(d=>{
...
function viewAdd(){
  return `<div class="body-pad">
    <div class="sec-title"><span class="diamond" style="padding:0;">✦</span>新增記錄</div>
    ...
    <div style="display:flex;gap:12px;">
      <div class="field" style="flex:1.3;"><label class="flabel">價格</label><input class="finput en" id="inPrice" placeholder="NT$ 0" inputmode="numeric"></div>
...
function signDone(){
  ...
  const prev = $('signPrev');
  prev.innerHTML = `<img src="${draft.sign}"><span class="zo">⛶</span>`;
...
function viewToday(){
  ...
  return `<div class="body-pad">
    <div class="sec-title"><span class="diamond" style="padding:0;">✦</span>今日營業額</div>
    <div class="stat-big">
      <div class="lb">TODAY · ${t.slice(5)}</div>
      <div class="num">NT$ ${fmtMoney(total)}</div>
      <div class="cnt">今日 ${recs.length} 筆服務</div>
      <div style="position:absolute;top:18px;right:20px;color:#3a3a3a;font-size:13px;">✦ ✦</div>
    </div>
    <div class="stat-row">
    ...
    <div class="sec-title" style="margin:24px 0 12px;font-size:11px;letter-spacing:0.28em;color:#777;font-weight:400;">今日記錄</div>
    ${list}
  </div>`;
}
...
function viewMonth(){
  return `<div class="body-pad">
    <div class="sec-title"><span class="diamond" style="padding:0;">✦</span>月結報表</div>
    <div class="msel">
...
function renderMonth(){
  ...
  $('mBody').innerHTML = `
    ...
    <div class="sec-title" style="margin:24px 0 6px;font-size:11px;letter-spacing:0.28em;color:#777;font-weight:400;">服務項目統計</div>
    ${svcRows}
    <div class="sec-title" style="margin:24px 0 12px;font-size:11px;letter-spacing:0.28em;color:#777;font-weight:400;">本月明細</div>
    ${list}
  `;
}
...
function runSearch(){
  ...
  const recs = DB.getRecords().filter(r=>
  ...
  Object.keys(byName).forEach(k=>{
    ...
    html += `<div style="margin-bottom:18px;">
      <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:8px;">
        <div><span style="color:#f0f0f0;font-size:16px;font-weight:500;">${nm}</span>
        <span class="search-result-meta">${ph||''}</span></div>
        <div style="font-size:11px;color:#777;">累計 <span class="en" style="color:#ccc;">$${fmtMoney(sum)}</span> · ${list.length}次</div>
      </div>
...
function openRec(id){
  ...
  $('recModalBody').innerHTML = `
    ...
    ${r.sign?`<div style="margin-top:14px;"><div class="flabel">簽名</div><div style="background:#f4f4f4;border-radius:8px;padding:8px;text-align:center;"><img src="${r.sign}" style="max-height:70px;max-width:100%;"></div></div>`:''}
    ...
    <button class="btn-ghost" style="width:100%;margin-top:10px;" onclick="closeRec()">關閉</button>
  `;
}
function closeRec(){ $('recMask').classList.remove('active'); }
function delRec(id){
  if(confirm('確定要刪除這筆記錄嗎？')){ DB.deleteRecord(id); closeRec(); toast('已刪除'); nav(document.querySelector('.di.on').dataset.view||'today'); }
}
function editRec(id){
  ...
  $('recModalBody').innerHTML = `
    ...
    <button class="btn-ghost" style="width:100%;margin-top:10px;" onclick="openRec('${r.id}')">返回</button>
  `;
}
function saveEdit(id){
  const patch = {
    ...
    price: parseInt(($('ePrice').value||'').replace(/[^0-9]/g,''))||0
  };
...
function viewExport(){
  return `<div class="body-pad">
    ...
    <div style="color:#999;font-size:13px;line-height:1.8;margin-bottom:20px;">把記錄匯出成 Excel 檔，可存到手機或上傳雲端備份。</div>
...
function doExport(){
  ...
  const rows = recs.map(r=>({
    日期:r.date, 時間:r.time, 姓名:r.name, 電話:r.phone||'',
    服務項目:r.services.join('、'), 價格:r.price, 已簽名:r.sign?'是':'否'
  }));
  const ws = XLSX.utils.json_to_sheet(rows);
  ws['!cols'] = [{wch:12},{wch:8},{wch:10},{wch:14},{wch:18},{wch:10},{wch:8}];
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, 'XIUGU 記錄');
